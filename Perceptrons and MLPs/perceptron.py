...
You can add any functions that will help you. The given function headers are used for testing the code, 
so changing them will fail testing.
"""
...
def featurize_data(
    ...
    ret_list : List[DataPointWithFeatures] = []
    
    for d in data:
        features = featurize(d.text)
        ret_list.append(DataPointWithFeatures(
            d.id, d.text, d.label, features
        ))
...
class PerceptronModel:
    ...
    def score(self, datapoint: DataPointWithFeatures, label: str) -> float:
        ...
        """
        total = 0.0
        for name, value in sorted(datapoint.features.items()):
            key = self._get_weight_key(name, label)
    ...
    def update_parameters(self, datapoint: DataPointWithFeatures, prediction: str, lr: float) -> None:
        ...
        for name, value in sorted(datapoint.features.items()):
            true_key = self._get_weight_key(name, true_label)
            self.weights[true_key] += lr * value

            pred_key = self._get_weight_key(name, prediction)
    ...
    def train(
        ...
        for _ in range(num_epochs):
            for data in training_data:
                ...
                self.update_parameters(data, self.predict(data), lr)
        
        self.save_weights(" ")
        print(self.evaluate(val_data))
