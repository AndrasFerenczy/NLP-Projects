...
def compute_records(processed_qs: List[str]):
    ...
    num_threads = 10
    timeout_secs = 120
    # Per-query safety limits (prevents a single bad query from freezing evaluation).
    # If a query is too slow, sqlite will be interrupted and we return an empty record set.
    per_query_timeout_secs = 10
    # Prevent pathological queries from materializing extremely large result sets.
    max_rows = 10_000
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as pool:
        for i, query in enumerate(processed_qs):
            futures.append(
                pool.submit(
                    ...
                    i,
                    query,
                    per_query_timeout_secs=per_query_timeout_secs,
                    max_rows=max_rows,
                )
            )
        ...
        try:
            for future in tqdm(
                as_completed(futures, timeout=timeout_secs),
                total=len(futures),
            ):
                query_id, rec, error_msg = future.result()
                rec_dict[query_id] = (rec, error_msg)
        except Exception:
            # Best-effort cancel. Note: already-running sqlite queries can't be force-killed
            # from another thread, hence the per-query progress-handler timeout above.
...
def compute_record(query_id, query, *, per_query_timeout_secs: int | None = None, max_rows: int | None = None):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    start = time.monotonic()

    def _progress_handler():
        if per_query_timeout_secs is None:
            return 0
        if (time.monotonic() - start) > per_query_timeout_secs:
            return 1  # non-zero => interrupt
        return 0

    if per_query_timeout_secs is not None:
        # Call handler every N VM instructions (tradeoff: overhead vs responsiveness).
    ...
    try:
        ...
        if max_rows is None:
            ...
            # Pull only up to max_rows+1 to detect pathological result sets.
            rows = cursor.fetchmany(max_rows + 1)
            if len(rows) > max_rows:
                rec = []
                error_msg = f"RuntimeError: result set exceeded {max_rows} rows"
            else:
                rec = rows
                error_msg = ""
    except Exception as e:
        rec = []
        # Normalize interrupted queries to a timeout message.
