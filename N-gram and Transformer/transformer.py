import os
import sys
import argparse
from typing import Dict, List
from tqdm import tqdm
import random
import pickle

import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
import math

from load_data_transformer import load_data_transformer as load_data
from perplexity import evaluate_perplexity
from wer import rerank_sentences_for_wer, compute_wer

if torch.cuda.is_available():
    DEVICE = torch.device("cuda")
elif getattr(torch.backends, "mps", None) is not None and torch.backends.mps.is_available():
    DEVICE = torch.device("mps")
else:
    DEVICE = torch.device("cpu")

# Shape Suffix Notation:
# B = Batch size
# L = Sequence length
# H = Hidden dimension
# V = Vocabulary size
# Q = Query/key/value dimension
# 1 = Singleton dimension (for broadcasting)

class CharacterLevelTransformer(nn.Module):
    """
    For this part of the assignment, we provide you with a skeleton for the Transformer
    decoder. However, we've introduced numerous errors to the code! The model currently compiles,
    but performs the incorrect computations. You must fix them to pass the unit tests.

    You may introduce additional keyword arguments after fixing the transformer, as long as the
    default behavior does not stray from the skeleton provided to you.
    """
    def __init__(self, num_layers: int, hidden_dim: int, num_heads: int,
                 ff_dim: int, dropout: float, vocab_size: int, *, vocab: Dict | None = None,
                 padding_idx: int | None = None):
        super().__init__()
        self.hidden_dim = hidden_dim
        if padding_idx is None:
            padding_idx = vocab_size - 1
        self.embed = nn.Embedding(vocab_size, hidden_dim, padding_idx=padding_idx)
        self.pos_embed = PositionalEncoding(hidden_dim, dropout)
        self.decoder = Decoder(num_layers, hidden_dim, num_heads, ff_dim, dropout)
        self.lm_head = nn.Linear(hidden_dim, vocab_size)
        self.padding_idx = int(padding_idx)

        self.vocab = vocab or {}
        self.stoi = self.vocab.get("stoi")
        self.itos = self.vocab.get("itos")
        self.bos_id = self.vocab.get("bos_id")
        self.eos_id = self.vocab.get("eos_id")
        self.unk_id = self.vocab.get("unk_id")

    def log_probability(self, input_tokens_BL, target_tokens_BL=None, base=np.e):
        """
        Computes the log-probabilities for the inputs in the given minibatch.

        Input:
            input_tokens_BL (torch.Tensor): A tensor of shape (B, L), where B is the 
                                         batch-size and L is the input length. 
            target_tokens_BL (torch.Tensor): A tensor of shape (B, L). For a given (i, j),
                                          target_tokens[i, j] should be the token following
                                          input_tokens[i, j]
        Output (torch.Tensor): A tensor of shape (B,) containing the log-probability for each
                               example in the minibatch
        """
        if target_tokens_BL is None:
            return float(self._sentence_log_probability(input_tokens_BL, base=base))

        model_device = next(self.parameters()).device
        input_tokens_BL = input_tokens_BL.to(model_device)
        target_tokens_BL = target_tokens_BL.to(model_device)
        logits_BLV = self.forward(input_tokens_BL)
        log_probs_BLV = F.log_softmax(logits_BLV, dim=-1)

        target_log_probs_BL = torch.gather(
            log_probs_BLV, dim=-1, index=target_tokens_BL.unsqueeze(-1)
        ).squeeze(-1)

        valid_BL = target_tokens_BL != self.padding_idx
        target_log_probs_BL = torch.where(valid_BL, target_log_probs_BL, torch.zeros_like(target_log_probs_BL))

        denom = float(np.log(base))
        if denom != 1.0:
            target_log_probs_BL = target_log_probs_BL / denom

        return torch.sum(target_log_probs_BL, dim=-1)

    def _sentence_log_probability(self, tokenized_sentence, base=np.e) -> float:
        if tokenized_sentence is None:
            return 0.0

        if len(tokenized_sentence) == 0:
            return 0.0

        if isinstance(tokenized_sentence[0], int):
            ids = list(tokenized_sentence)
        else:
            if self.stoi is None:
                raise TypeError(
                    "CharacterLevelTransformer.log_probability(list[str]) requires a vocab with `stoi`."
                )
            unk_id = self.unk_id if self.unk_id is not None else self.padding_idx
            ids = [self.stoi.get(ch, unk_id) for ch in tokenized_sentence]

        if self.bos_id is not None:
            ids = [int(self.bos_id)] + ids
        if self.eos_id is not None:
            ids = ids + [int(self.eos_id)]

        if len(ids) < 2:
            return 0.0

        model_device = next(self.parameters()).device
        input_ids = torch.tensor([ids[:-1]], dtype=torch.long, device=model_device)
        target_ids = torch.tensor([ids[1:]], dtype=torch.long, device=model_device)
        return float(self.log_probability(input_ids, target_ids, base=base).item())

    def forward(self, model_input_BL):
        # Perform the embedding
        embeds_BLH = self.embed(model_input_BL) * math.sqrt(self.hidden_dim)
        embeds_BLH = self.pos_embed(embeds_BLH)

        # Pass through the decoder  
        mask_1LL = construct_self_attn_mask(model_input_BL)
        decoder_output_BLH = self.decoder(embeds_BLH, mask_1LL)
        output_BLV = self.lm_head(decoder_output_BLH)
        return output_BLV

    @torch.no_grad()
    def generate(self, num_samples: int, max_steps: int, save_path: str, *, temperature: float = 1.0):
        if self.itos is None:
            raise ValueError("CharacterLevelTransformer.generate() requires `vocab['itos']`.")

        os.makedirs(os.path.dirname(save_path) or ".", exist_ok=True)
        model_device = next(self.parameters()).device

        pad_id = int(self.padding_idx)
        bos_id = int(self.bos_id) if self.bos_id is not None else None
        eos_id = int(self.eos_id) if self.eos_id is not None else None

        self.eval()
        generations: List[str] = []
        for _ in range(int(num_samples)):
            ids: List[int] = []
            if bos_id is not None:
                ids.append(bos_id)
            else:
                ids.append(0)

            for _ in range(int(max_steps)):
                input_ids = torch.tensor([ids], dtype=torch.long, device=model_device)
                logits = self.forward(input_ids)[:, -1, :]
                logits[:, pad_id] = float("-inf")
                if temperature and temperature != 1.0:
                    logits = logits / float(temperature)
                probs = F.softmax(logits, dim=-1)
                next_id = int(torch.multinomial(probs, num_samples=1).item())
                ids.append(next_id)
                if eos_id is not None and next_id == eos_id:
                    break

            toks = []
            for tok_id in ids:
                if bos_id is not None and tok_id == bos_id:
                    continue
                if eos_id is not None and tok_id == eos_id:
                    break
                tok = self.itos[tok_id]
                if len(tok) == 1:
                    toks.append(tok)
            generations.append("".join(toks))

        with open(save_path, "wb") as f:
            pickle.dump(generations, f)

def construct_self_attn_mask(x_BL: torch.Tensor):
    """
    Constructs a causal self-attention mask. The output to this function should be a mask of shape
    (1, L, L). Indices that a token can attend to should be
    set to true.

    - Row index (i): Represents the token position.
    - Column index (j): Represents whether token i can attend to token j.
    - A token at position i can only attend to tokens ≤ i.

    There are two errors in this function.
    """
    L = x_BL.size(1)
    mask_LL = torch.tril(torch.ones(L, L, device=x_BL.device, dtype=torch.bool))
    return mask_LL.unsqueeze(0)

class Decoder(nn.Module):
    def __init__(self, num_layers, hidden_dim, num_heads, ff_dim, dropout):
        """
        There is a single error in this function that will prevent the model from learning.
        """
        super().__init__()
        self.layers = nn.ModuleList(
            [TransformerBlock(num_heads, hidden_dim, ff_dim, dropout) for _ in range(num_layers)]
        )
        
    def forward(self, x_BLH, mask_1LL):
        for layer in self.layers:
            x_BLH = layer(x_BLH, mask_1LL)
        return x_BLH

class TransformerBlock(nn.Module):
    def __init__(self, num_heads, hidden_dim, ff_dim, dropout):
        super().__init__()

         # Attention block
        self.attn_block = MultiHeadAttention(num_heads, hidden_dim, dropout)
        self.attn_dropout = nn.Dropout(dropout) 
        self.attn_norm = nn.LayerNorm(hidden_dim)

        # Feedforward block
        self.mlp_block = TransformerMLP(hidden_dim, ff_dim, dropout)
        self.mlp_dropout = nn.Dropout(dropout) 
        self.mlp_norm = nn.LayerNorm(hidden_dim)

    def forward(self, x_BLH, mask_1LL):
        """
        There are two types of errors in this function.
        """
        attn_out_BLH = self.attn_block(x_BLH, mask_1LL)
        x_BLH = self.attn_norm(x_BLH + self.attn_dropout(attn_out_BLH))

        mlp_out_BLH = self.mlp_block(x_BLH)
        x_BLH = self.mlp_norm(x_BLH + self.mlp_dropout(mlp_out_BLH))
        return x_BLH

class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads, hidden_dim, dropout=0.1):
        super().__init__()
        self.h = num_heads
        self.qkv_dim = hidden_dim // num_heads
        self.dropout = nn.Dropout(dropout)
        
        self.q_proj = nn.Linear(hidden_dim, hidden_dim)
        self.k_proj = nn.Linear(hidden_dim, hidden_dim)
        self.v_proj = nn.Linear(hidden_dim, hidden_dim)
        self.out_proj = nn.Linear(hidden_dim, hidden_dim)

    def attention(self, query_BHQL, key_BHQL, value_BHQL, mask_1LL):
        """
        There are three errors in this function to fix.
        """
        dot_products_BHLL = torch.matmul(query_BHQL, key_BHQL.transpose(-2, -1)) / math.sqrt(
            self.qkv_dim
        )
        dot_products_BHLL = dot_products_BHLL.masked_fill(mask_1LL == 0, -1e9)
        attn_BHLL = self.dropout(F.softmax(dot_products_BHLL, dim=-1))
        return torch.matmul(attn_BHLL, value_BHQL)

    def forward(self, x_BLH, mask_1LL):
        """
        There are two errors in this function to fix
        """
        mask_B1LL = mask_1LL.unsqueeze(1)
        B = x_BLH.size(0)
        
        query_BHQL = self.q_proj(x_BLH).view(B, -1, self.h, self.qkv_dim).transpose(1, 2)
        key_BHQL = self.k_proj(x_BLH).view(B, -1, self.h, self.qkv_dim).transpose(1, 2)
        value_BHQL = self.v_proj(x_BLH).view(B, -1, self.h, self.qkv_dim).transpose(1, 2)
        
        x_BHQL = self.attention(query_BHQL, key_BHQL, value_BHQL, mask_B1LL)
        
        x_BLH = x_BHQL.transpose(1, 2).contiguous().view(B, -1, self.h * self.qkv_dim)
        return self.out_proj(x_BLH)

class TransformerMLP(nn.Module):
    def __init__(self, hidden_dim, ff_dim, dropout=0.1):
        super().__init__()
        self.fc1 = nn.Linear(hidden_dim, ff_dim)
        self.fc2 = nn.Linear(ff_dim, hidden_dim)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x_BLH):
        """
        There is a single error in this function to fix.
        """
        return self.fc2(self.dropout(F.relu(self.fc1(x_BLH))))

class PositionalEncoding(nn.Module):
    def __init__(self, hidden_dim, dropout, max_len=1000):
        super().__init__()
        self.dropout = nn.Dropout(p=dropout)
        
        positional_encodings_LH = torch.zeros(max_len, hidden_dim)
        position_L1 = torch.arange(0, max_len).unsqueeze(1)
        div_term_H = torch.exp(torch.arange(0, hidden_dim, 2) * (- math.log(10000) / hidden_dim))
        positional_encodings_LH[:, 0::2] = torch.sin(position_L1 * div_term_H)
        positional_encodings_LH[:, 1::2] = torch.cos(position_L1 * div_term_H)
        positional_encodings_1LH = positional_encodings_LH.unsqueeze(0)
        
        self.register_buffer('positional_encodings', positional_encodings_1LH, persistent=False)

    def forward(self, x_BLH):
        x_BLH = x_BLH + self.positional_encodings[:, :x_BLH.size(1)]
        return self.dropout(x_BLH)

def train(
    model: CharacterLevelTransformer,
    train_loader,
    val_loader,
    *,
    learning_rate: float,
    max_epochs: int,
    grad_clip: float | None = None,
    eval_every: int = 1,
    checkpoint_path: str | None = None,
):
    optimizer = torch.optim.AdamW(model.parameters(), lr=float(learning_rate))
    loss_fct = nn.CrossEntropyLoss(ignore_index=int(model.padding_idx))

    model_device = next(model.parameters()).device
    best_val_ppl = float("inf")
    best_state = None

    for epoch in range(1, int(max_epochs) + 1):
        model.train()
        total_loss = 0.0
        total_steps = 0

        for input_tokens_BL, target_tokens_BL in tqdm(train_loader, desc=f"epoch {epoch}", leave=False):
            input_tokens_BL = input_tokens_BL.to(model_device)
            target_tokens_BL = target_tokens_BL.to(model_device)

            optimizer.zero_grad(set_to_none=True)
            logits_BLV = model(input_tokens_BL)
            loss = loss_fct(logits_BLV.view(-1, logits_BLV.size(-1)), target_tokens_BL.view(-1))
            loss.backward()
            if grad_clip is not None and float(grad_clip) > 0.0:
                torch.nn.utils.clip_grad_norm_(model.parameters(), float(grad_clip))
            optimizer.step()

            total_loss += float(loss.item())
            total_steps += 1

        avg_loss = total_loss / max(1, total_steps)
        print(f"epoch {epoch} train_loss={avg_loss:.4f}")

        if val_loader is not None and int(eval_every) > 0 and epoch % int(eval_every) == 0:
            model.eval()
            val_ppl = float(evaluate_perplexity(model, val_loader))
            print(f"epoch {epoch} val_ppl={val_ppl:.4f}")

            if val_ppl < best_val_ppl:
                best_val_ppl = val_ppl
                best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
                if checkpoint_path is not None:
                    os.makedirs(os.path.dirname(checkpoint_path) or ".", exist_ok=True)
                    torch.save(best_state, checkpoint_path)

    if best_state is not None:
        model.load_state_dict(best_state)
    return best_val_ppl

def get_args():
    """
    You may freely add new command line arguments to this function.
    """
    parser = argparse.ArgumentParser(description='Transformer model')
    parser.add_argument('--num_layers', type=int, default=6,
                        help="How many transformer blocks to use")
    parser.add_argument('--hidden_dim', type=int, default=512,
                        help="What is the transformer hidden dimension")
    parser.add_argument('--num_heads', type=int, default=8,
                        help="How many heads to use for Multihead Attention")
    parser.add_argument('--ff_dim', type=int, default=2048,
                        help="What is the intermediate dimension for the feedforward layer")
    parser.add_argument('--dropout_p', type=float, default=0.1,
                        help="The dropout probability to use")    

    parser.add_argument('--seq_len', type=int, default=128,
                        help="Sequence length for character LM batches")
    parser.add_argument('--batch_size', type=int, default=64,
                        help="Batch size for training/eval")
    parser.add_argument('--seed', type=int, default=0,
                        help="Random seed for reproducibility")

    parser.add_argument('--learning_rate', type=float, default=3e-4,
                        help="AdamW learning rate")
    parser.add_argument('--max_epochs', type=int, default=5,
                        help="Number of epochs to train")
    parser.add_argument('--grad_clip', type=float, default=1.0,
                        help="Global grad norm clipping (<=0 disables)")
    parser.add_argument('--eval_every', type=int, default=1,
                        help="Evaluate val perplexity every N epochs")
    parser.add_argument('--no_train', action='store_true',
                        help="Skip training (load data + run eval only)")
    parser.add_argument('--load_checkpoint', type=str, default=None,
                        help="Path to a model checkpoint (state_dict) to load before evaluation")
    parser.add_argument('--no_generation', action='store_true',
                        help="Skip sampling/generation step")

    parser.add_argument('--experiment_name', type=str, default='transformer_')
    parser.add_argument('--num_samples', type=int, default=10,
                        help="How many samples should we get from our model??")
    parser.add_argument('--max_steps', type=int, default=40,
                        help="What should the maximum output length be?")
    parser.add_argument('--temperature', type=float, default=1.0,
                        help="Sampling temperature for generation")
    

    args = parser.parse_args()
    return args

def main():
    # Get key arguments
    args = get_args()

    random.seed(args.seed)
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)

    # Get the data
    train_loader, val_loader, dev_loader, test_loader, dev_wer_data, test_wer_data, vocab = load_data(
        "character", seq_len=args.seq_len, batch_size=args.batch_size, seed=args.seed
    )

    # Initialize the transformer and train
    num_layers = args.num_layers
    hidden_dim = args.hidden_dim
    num_heads = args.num_heads
    ff_dim = args.ff_dim
    dropout_p = args.dropout_p
    vocab_size = len(vocab["itos"])
    model = CharacterLevelTransformer(num_layers, hidden_dim, num_heads, ff_dim,
                                      dropout_p, vocab_size, vocab=vocab, padding_idx=vocab["pad_id"]).to(DEVICE)

    if args.load_checkpoint:
        state_dict = torch.load(args.load_checkpoint, map_location="cpu")
        model.load_state_dict(state_dict)

    if not args.no_train:
        ckpt_path = os.path.join("results", f"{args.experiment_name}best_transformer.pth")
        train(
            model,
            train_loader,
            val_loader,
            learning_rate=args.learning_rate,
            max_epochs=args.max_epochs,
            grad_clip=args.grad_clip,
            eval_every=args.eval_every,
            checkpoint_path=ckpt_path,
        )

    # Evaluate model perplexity
    model.eval()
    val_perplexity = evaluate_perplexity(model, val_loader)
    print(f'Model perplexity on the val set: {val_perplexity}')
    dev_perplexity = evaluate_perplexity(model, dev_loader)
    print(f'Model perplexity on the dev set: {dev_perplexity}')
    test_perplexity = evaluate_perplexity(model, test_loader)
    print(f'Model perplexity on the test set: {test_perplexity}')    
    
    # Evaluate model WER
    experiment_name = args.experiment_name
    dev_wer_savepath = os.path.join('results', f'{experiment_name}transformer_dev_wer_predictions.csv')
    rerank_sentences_for_wer(model, dev_wer_data, dev_wer_savepath) 
    dev_wer = compute_wer('data/wer_data/dev_ground_truths.csv', dev_wer_savepath)
    print("Dev set WER was: ", dev_wer)

    test_wer_savepath = os.path.join('results', 'transformer_test_wer_predictions.csv')
    rerank_sentences_for_wer(model, test_wer_data, test_wer_savepath)
    
    # Generate text from the model
    if not args.no_generation:
        generation_path = os.path.join('generations', f'{experiment_name}transformer_generation_examples.pkl')
        model.generate(args.num_samples, args.max_steps, generation_path, temperature=args.temperature)


if __name__ == "__main__":
    main()
